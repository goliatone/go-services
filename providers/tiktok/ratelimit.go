...
const defaultRetryAfter429 = 3 * time.Second
...
func NormalizeAPIResponse(_ context.Context, response core.TransportResponse) (core.ProviderResponseMeta, error) {
	...
	if requestID := headerValue(meta.Headers, "x-tt-logid"); requestID != "" {
		meta.Metadata["tiktok_request_id"] = requestID
	}
	if traceID := headerValue(meta.Headers, "x-tt-trace-id"); traceID != "" {
		meta.Metadata["tiktok_trace_id"] = traceID
	}

	if limit, ok := parseHeaderInt(meta.Headers, "x-ratelimit-limit"); ok {
		meta.Headers["X-RateLimit-Limit"] = strconv.Itoa(limit)
		meta.Metadata["tiktok_rate_limit"] = limit
	}
