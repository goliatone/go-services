...
func TestNew_UsesOAuthDefaults(t *testing.T) {
	...
	for _, scope := range []string{ScopeUserInfoBasic, ScopeVideoList, ScopeVideoInsights} {
		if !scopeSet[scope] {
...
func TestProvider_BaselineCapabilitiesUseCanonicalRequiredGrants(t *testing.T) {
	...
	expected := map[string]string{
		"profile.read":   GrantUserInfoBasic,
		"video.read":     GrantVideoList,
		"analytics.read": GrantVideoInsights,
	}
...
func TestProvider_CompleteAuthAndRefresh(t *testing.T) {
	...
	complete, err := provider.CompleteAuth(context.Background(), core.CompleteAuthRequest{
		...
		Code:     "code_1",
		State:    "state_1",
		Metadata: map[string]any{"external_account_id": "tik_tok_user_1"},
	})
	if err != nil {
		t.Fatalf("complete auth: %v", err)
	}
	if complete.ExternalAccountID != "tik_tok_user_1" {
		t.Fatalf("expected external account id tik_tok_user_1, got %q", complete.ExternalAccountID)
	}
...
func TestProvider_NormalizeGrantedPermissions(t *testing.T) {
	...
	expected := []string{GrantUserInfoBasic, GrantVideoInsights, GrantVideoList}
