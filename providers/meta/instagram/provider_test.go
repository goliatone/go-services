...
func TestNew_UsesSharedMetaAuthDefaults(t *testing.T) {
	...
	for _, scope := range []string{ScopeInstagramBasic, ScopeInstagramManageInsights, ScopePagesShowList} {
		if !scopeSet[scope] {
...
func TestProvider_BaselineCapabilitiesUseCanonicalRequiredGrants(t *testing.T) {
	...
	expected := map[string]string{
		"media.read":        GrantInstagramBasic,
		"insights.read":     GrantInstagramManageInsights,
		"page.binding.read": GrantPagesShowList,
	}
...
func TestProvider_NormalizeGrantedPermissions(t *testing.T) {
	...
	expected := []string{GrantInstagramBasic, GrantInstagramManageInsights, GrantPagesShowList}
